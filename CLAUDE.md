...
## Future phase: Appointment Tracking (Phase 10 — not yet started)
Decided 2026-08-25, inspired by competitor "Quell" (meetquell.com) — their "Appointments" page detects booking/appointment confirmations across the inbox and maps them onto a real calendar view, with a day-before reminder banner. Do not build any part of this until Phases 1-9 are live and proven, per the sequencing rule above — this section exists to capture the design intent now, before it's forgotten, not to greenlight starting it.

- **Scope:** a second Claude pass (or a classification sub-type) detects appointment/booking confirmation emails (medical, salon, fitness, service bookings, travel check-ins, etc.) and extracts date, time, business name, and a category tag (Health, Fitness, Hair & Beauty, etc.) live per email.
- **Delivery:** a dedicated "Appointments" view — an upcoming list plus a real calendar widget — with a day-before reminder. Reminder delivery mechanism (in-app only vs. email vs. push) is not decided yet, the same open question already flagged for Phase 6's deadline nudges.
- **Zero-content-storage tension, same class of question already flagged for Phases 6 and 8:** extracting and displaying date/time/business name means either fetching live from Gmail at render time (compatible with the existing hard rule, same pattern `gmailDisplay.ts` already uses) or storing structured appointment data (a real decision, not yet made). Needs its own explicit resolution before this phase starts, not a silent assumption.

## Future phase: AI Chat Assistant (Phase 11 — not yet started)
Decided 2026-08-25, inspired by both competitors — Quell's "Ask Quell or search your inboxes" search bar and Tame My Inbox's "IRIS Chat." A conversational interface for querying/acting on your inbox in natural language. Distinct from Phase 9's plain-English custom rules (which only ever produces a structured classification rule) — this would be open-ended Q&A and/or actions over the inbox, a different and larger surface. Do not build any part of this until Phases 1-10 are live and proven, per the sequencing rule above — captured now so the idea isn't lost, not a commitment to build it soon.

- **Scope not yet defined** — needs its own scoping pass when its turn comes. Open questions to resolve then, not now: is this read-only Q&A over already-stored classification metadata, or does it get live per-query access to email content the way classification does (and if so, under what version of the zero-content-storage rule); does it just answer questions, or can it trigger real actions (Phase 4's archive, Phase 3's draft creation)?
- **Relationship to existing pieces:** likely sits on top of already-planned mechanisms (Phase 3's draft generation, Phase 9's rule parsing) rather than being a wholly separate system — worth revisiting how much this subsumes once those phases actually exist, instead of designing it in isolation now.
- **Not a voice assistant** — this is text chat. The project's existing "no voice assistant planned" note is about voice specifically and doesn't apply here; worth being precise about that distinction if this phase is ever scoped further.
