...
1. priority: true if EITHER of these is true:
   - It's a real question, request, or matter from a person genuinely expecting a reply or action. This alone is always enough — true regardless of urgency, effort to answer, or whether they say "no rush" or "take your time." The "can wait indefinitely" false-condition below does NOT apply to this case; a real question from a person only skips priority if no reply is genuinely expected at all (a thank-you needing no response, a pure FYI, an auto-reply). OR
   ...
   false for: a person's message where no reply is genuinely expected at all (a thank-you, a pure FYI, an auto-reply) — never for a real question just because it's phrased casually or without urgency; and automated messages with no real deadline or consequence for waiting. This specifically includes routine "confirm your email" / "verify your account to finish setup" messages: even though signing up is something the recipient is actively doing, the confirmation link itself has no stated expiry and no real cost to waiting — they can click it anytime without losing anything. That's different from a login code (needed right this moment to get into an account they're actively trying to access right now) or a deadline-bound notice (a real cost if missed) — both of those stay priority=true.
